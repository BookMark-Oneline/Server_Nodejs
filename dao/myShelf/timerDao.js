...
module.exports.updateOnline = async (connection, user_id) => {
  const updateOnlineQuery = `UPDATE UserInfo SET now_reading=? WHERE UserInfo.user_id=?;`;
...
module.exports.selectTotalReadingTime = async (
  ...
  const [selectTotalReadingTimeRow, selectTotalReadingTimeFields] =
    await connection.query(selectTotalReadingTimeQuery, [user_id, book_id]);

  return selectTotalReadingTimeRow;
};

// 현재 유저가 선택한 책의 총 독서시간과 현재 읽고있는 페이지 업데이트
module.exports.updateBookInfo = async (
  connection,
  total_reading_time,
  ...
  const updateBookInfoRow = await connection.query(updateBookInfoQuery, [
    total_reading_time,
    parseInt(current_reading_page),
    user_id,
    book_id,
  ]);
  return updateBookInfoRow[0];
};
