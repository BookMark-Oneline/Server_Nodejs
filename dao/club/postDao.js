...
module.exports.clubMember = async (connection, [club_id]) =>{
    const clubSearchQuery =`SELECT DISTINCT UserInfo.user_id, UserInfo.now_reading, UserInfo.profile_message FROM UserInfo LEFT JOIN ClubMember ON UserInfo.user_id = ClubMember.user_id LEFT JOIN ClubInfo ON ClubMember.club_id = ClubInfo.club_id WHERE ClubInfo.club_id = ?;`;
...
module.exports.clubSearch = async (connection, [club_id]) =>{
    const clubSearchQuery =`SELECT ClubInfo.club_id, ClubInfo.club_img_url, ClubInfo.club_invite_option FROM ClubInfo WHERE ClubInfo.club_id = ?;`;
    const clubSearchRow = await connection.query(clubSearchQuery, [club_id]);
